...
G2048.prototype={
    ...
    arrValueUpdate:function(num,i,j){
        //更新一个数组的值
        this.arr[i][j].oldValue=this.arr[i][j].calue;
    ...
    moveDown:function(){
            ...
            for(j=3;j>=0;j--){
                ...
                while(k<=n){
                    if(this.arr[i][k].value==0){
                        if(k==n||(this.arr[i][k+1].value!=0&&this.arr[i][k+1].value!=this.ar[i][j].value)){
        ...
        this.newCell;//生成一个新的格子，后面要对其做判断
    },
    moveUp:function(){
        //向上移动
        var i,j,k,n;
    ...
    moveLeft:function(){
        ...
        for(j=0;j<4;j++){
            ...
            for(i=0;j<4;i++){
                ...
                while(k>=n){
                    if(this.arr[i][j].value==0){
                        if(k==n||(this.arr[k-1][j].value!=0&&this.arr[k-1][j].value!=this.ar[i][j].value)){
                            this.moveCell(i,j,i,k);
                        }
                        k--;
                    }else{
    ...
    mergeCells:function(i1,j1,i2,j2){
        ...
        var theDom=$("p"+i1+j1).addClass("p"+i2+j2).find('.number_cell_con');
        setTimeout(function(){
